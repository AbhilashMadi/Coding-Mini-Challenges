...
for (let i = 0; i < clockDigits.length; i++) {
  ...
  digitsEl.style.left =
    50 + Math.sin(((Math.PI * 2) / clockDigits.length) * i + 0.5) * 45 + "%";
  digitsEl.style.top =
    50 - Math.cos(((Math.PI * 2) / clockDigits.length) * i + 0.5) * 45 + "%";
...
const getTimeInAngles = (date) => {
  return {
    ...
    minutes:
      (clockDigits.length / 2) * date.getMinutes() + date.getSeconds() / 10,
